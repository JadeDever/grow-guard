...
const RiskManagementPanel: React.FC<RiskManagementPanelProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'alerts' | 'suggestions' | 'reports'>('overview');
      ...
      <div className="border-b border-gray-200 mb-6">
        <nav className="flex space-x-8">
          {[
            ...
            { id: 'alerts', label: '风险警报', icon: AlertTriangle, count: stopLossAlerts.length + takeProfitAlerts.length },
            { id: 'suggestions', label: '调仓建议', icon: TrendingUp, count: rebalanceSuggestions.length },
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-lg">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-lg font-medium text-gray-900 mb-2">整体风险等级</h3>
                <div className="flex items-center space-x-3">
                  <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getRiskLevelColor(riskAssessment.overallRiskLevel)}`}>
          ...
          {/* 赛道风险分析 */}
          <div>
            <h3 className="text-lg font-medium text-gray-900 mb-4">赛道风险分析</h3>
            <div className="space-y-3">
              {riskAssessment.sectorRisks.map((sector, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div className="flex items-center space-x-3">
                    <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getRiskLevelColor(sector.riskLevel)}`}>
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-gray-500">
                      {sector.positionCount}只股票 · {(sector.totalWeight * 100).toFixed(1)}%权重
                    </div>
                  </div>
                </div>
              ))}
          ...
          {/* 风险建议 */}
          <div>
            <h3 className="text-lg font-medium text-gray-900 mb-4">主要风险建议</h3>
        ...
        <div className="space-y-6">
                ...
                {stopLossAlerts.map((alert, index) => (
                  <div key={index} className="p-4 bg-red-50 border border-red-200 rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      <div className="font-medium text-red-900">{alert.stockName}</div>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <span className="ml-2 font-medium">¥{alert.stopLossPrice}</span>
                      </div>
                    </div>
              ...
              <div className="space-y-3">
                {takeProfitAlerts.map((alert, index) => (
                  <div key={index} className="p-4 bg-green-50 border border-green-200 rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      <div className="font-medium text-green-900">{alert.stockName}</div>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <span className="ml-2 font-medium">¥{alert.takeProfitPrice}</span>
                      </div>
                    </div>
          ...
          {stopLossAlerts.length === 0 && takeProfitAlerts.length === 0 && (
            <div className="text-center py-8">
              ...
              <div className="text-sm text-gray-400">当前所有持仓都在安全范围内</div>
            </div>
          )}
        </div>
      )}
      ...
      {activeTab === 'suggestions' && (
        <div className="space-y-6">
          {rebalanceSuggestions.length > 0 ? (
            <div>
              <h3 className="text-lg font-medium text-gray-900 mb-4">再平衡建议</h3>
                  ...
                  <div key={index} className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                        suggestion.type === 'reduce' ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800'
                      }`}>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm mb-2">
                      <div>
                        ...
                        <span className="ml-2 font-medium">{(suggestion.currentWeight * 100).toFixed(1)}%</span>
                      </div>
                      <div>
                        <span className="text-yellow-600">建议权重:</span>
                        <span className="ml-2 font-medium">{(suggestion.suggestedWeight * 100).toFixed(1)}%</span>
                      </div>
                    </div>
            ...
            <div className="text-center py-8">
              ...
              <div className="text-sm text-gray-400">当前持仓结构合理，无需调整</div>
            </div>
          )}
        </div>
      )}
